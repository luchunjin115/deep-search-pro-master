...
| 已完成到 | M1-19：最小Next.js登录、库存聊天和数据库Evidence侧栏 |
| 下一动作 | 用户理解并确认M1-19结果后，单独授权进入M1-20故障矩阵、集成和浏览器端到端回归 |
...
| M1 | 库存查询垂直切片 | 进行中 | [查看M1详细记录](progress/M1_INVENTORY_QUERY.md)；M1-01至M1-19已完成并验证 |
